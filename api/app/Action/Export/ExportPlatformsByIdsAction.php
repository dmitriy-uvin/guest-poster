...
final class ExportPlatformsByIdsAction
{
    public function execute(ByIdsRequest $request)
    {
        ...
        File::put(storage_path('app/public/export/export-by-ids.csv'),'');
        $f = fopen('../storage/app/public/export/export-by-ids.csv', 'a+');
        fputcsv($f, [
            'Protocol',
            'Website Url',
            'Organic Traffic',
        ...
        $platforms = Platform::whereIn('id', $request->getIds())->get();
        foreach ($platforms as $platform) {
            fputcsv($f, [
                $platform->protocol,
                $platform->website_url,
                $this->returnNotAvailable($platform->organic_traffic),
                (int)$platform->dofollow_active,
                (int)$platform->money_anchor,
                (int)$platform->free_home_featured_active,
                $platform->niche_edit_link_price,
                (int)$platform->niche_edit_link_active,
